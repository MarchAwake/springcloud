...
public class CustomRule extends AbstractLoadBalancerRule implements IRule {
    ...
    public Server route(int hashId, List<Server> addressList) {
        ...
        TreeMap<Long, Server> addressMap = new TreeMap<>();

        addressList.forEach(element -> {
            // 虚化将若干个服务节点分布到圆环上
            for (int i = 0; i < 8; i++) {
                long hash = hash(element.getId() + 1);
                addressMap.put(hash, element);
            }
        });

        long hash = hash(String.valueOf(hashId));
        SortedMap<Long, Server> last = addressMap.tailMap(hash);

        // 当request URL的hash值大于任意一个服务器对应的hashKey，
        // 取address中的第一个节点
        if (last.isEmpty()) {
            return addressMap.firstEntry().getValue();
        }
    ...
    private long hash(String key) {
        ...
        hashCode = ((long) (digest[2] & 0xFF << 16))
